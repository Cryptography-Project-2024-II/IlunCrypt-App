...
import com.iluncrypt.iluncryptapp.models.algorithms.symmetrickey.DESManager;
...
import static com.iluncrypt.iluncryptapp.models.algorithms.symmetrickey.DESManager.decryptFile;
...
public class DESController implements CipherController, Initializable {
    ...
    private void loadDESConfig() {
        ...
        boxIV.setVisible(desConfig.isShowIV());
        boxIV.setManaged(desConfig.isShowIV());
    }
    ...
    private void saveEncryptedInformation() {
        ...
        try {
            ...
            IlunFileMetadata metadata = new IlunFileMetadata(
                    desConfig.isSaveAlgorithm() ? desConfig.getAlgorithm().name() : null,
                    extension,
                    dataToSave.length,
                    checksum,
                    desConfig.isSaveAlgorithm()
            );
    ...
    private void uploadKey() {
        try {
            ...
            int keySizeBytes = desConfig.getKeySize().getSize() / 8;
    ...
    private void regenerateKey() {
        try {
            SecretKey generatedKey = DESManager.generateKey(desConfig);
    ...
    private void regenerateIV() {
        byte[] iv = new byte[desConfig.getMode().getFixedIVSize()];
        ...
        try {
            ...
            if (!keyText.isEmpty()) {
                try {
                    ...
                    int trueKeySizeBits = desConfig.getAlgorithm() != SymmetricKeyAlgorithm.DES ? desConfig.getKeySize().getSize(): 64;
                    if (keySizeBits == trueKeySizeBits) {
                        isValidKey = true;
                    }
            ...
            if (!isValidKey && desConfig.isGenerateKey()) {
                SecretKey generatedKey = DESManager.generateKey(desConfig);
            ...
            if(desConfig.getMode().requiresIV()) {
                if (desConfig.isGenerateIV() && textFieldIV.getText().trim().isEmpty()) {
                    iv = desConfig.getMode().requiresIV() ? DESManager.generateIV(desConfig) : null;

                    if (desConfig.isShowIV() && iv != null) {
                        textFieldIV.setText(Base64.getEncoder().encodeToString(iv));
                    }
                } else {
                    String ivText = textFieldIV.getText().trim();
                    ...
                    try {
                        ...
                        int expectedIVSize = desConfig.getMode().getFixedIVSize();
                        if (expectedIVSize == -1) {
                            expectedIVSize = desConfig.getAlgorithm().getBaseIVSize();
                        }
            ...
            if(fileMode){
                encryptedInfo = DESManager.encryptFile(decryptedFile,key,iv, desConfig);
                ...
                String encryptedText = DESManager.encryptText(plainText, key, iv, desConfig);
    ...
    private void decrypt(ActionEvent event) {
        try {
            ...
            if (fileMode) {
                ...
                decryptedInfo = decryptFile(encryptedFile, key, desConfig);
                ...
                String decryptedText = DESManager.decryptText(cipherText, key, desConfig);
    ...
    private void showInfoDialog() {
        ...
        infoDialog.showFXMLDialog(
                "Data Encryption Standard (DES) Information",
                ...
                controller -> {
                    ...
                    controller.loadContent("views/symmetric-key/des/des-description-view.fxml", bundle); // Load the description view inside the container
                }
        );
    }
